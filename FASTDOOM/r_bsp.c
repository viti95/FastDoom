...
void R_UpdateClipSolidWallSegment(int first,
                                  int last)
{
    cliprange_t *next;
    cliprange_t *start;
...
void R_UpdateClipPassWallSegment(int first,
                                 int last)
{
    cliprange_t *start;
...
void R_UpdateAddLine(seg_t *line)
{
    int x1;
    int x2;
    angle_t angle1;
...
clippass:
    R_UpdateClipPassWallSegment(x1, x2 - 1);
    return;

clipsolid:
    R_UpdateClipSolidWallSegment(x1, x2 - 1);
}
...
void R_UpdateBSPNode(int bspnum)
{
    ...
    while (true)
    {
        ...
        if (bspnum == -1)
            ...
            R_UpdateSubsector(bspnum & (~NF_SUBSECTOR));
