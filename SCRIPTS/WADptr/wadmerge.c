...
suggest()
{
	...
	for (count = 0; count < numentries; count++) {                                                    /* each lump in turn */
		for (count2 = 0; count2 < count; count2++) {                                                  /* check against previous */
			if ((wadentry[count].length == wadentry[count2].length) && wadentry[count].length != 0) { /* same length, might be same lump */
				if (linkcnt >= maxlinks) {
					maxlinks += MAXLINKS;
	...
	for (count = 0; count < linkcnt; count++) {
		...
		/* compare them */
		if (!memcmp(check1, check2, wadentry[links[2 * count]].length)) { /* they are the same ! */
