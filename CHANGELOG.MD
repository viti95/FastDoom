...
## 1.1.1 (?? ??? ????)
* Fixed VESA linear modes that are not 100% linear (partial empty scanlines)
* Added a new menu option to enable or disable realtime automap updates
* Improved the speed of realtime automap updates
