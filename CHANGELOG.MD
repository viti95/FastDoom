...
## 1.4.0 (?? ??? ????)
* Mouse buttons can now be bound to different actions: each of the 3 buttons can be set to Fire, Move Forward, Move Backward, Strafe, Use, Speed, Previous Weapon, Next Weapon or None #236
* Fixed mouse X axis sign bug (left mouse strafe now works properly)
* New command line parameter -gamemode to select the game mode ("shareware", "registered", "commercial" or "retail")
* New command line parameter -gamemission to select the game mission ("doom", "doom2", "tnt" or "plutonia")
* Command line parameter -complevel now validates its value (only 2, 3 or 4 are valid)
* Unknown IWADs loaded with -iwad command line parameter are now rejected unless -complevel, -gamemode and -gamemission are all specified
* New command line parameter that preloads the IWAD into RAM (-preload)
* Added secrets completed counter to the automap #276
* Added next/previous weapon keys #263
* Added 5×6 integer scaling for 320×200 VESA backbuffer modes, optimized for 1600×1200 (4:3) displays while preserving the original aspect ratio.
* New executable: fdmvt100.exe (VT100 terminal output support via serial port, display only)
* New executable: fdoompgc.exe (IBM Professional Graphics Controller, 640×480 256 colors)
* IBM Music Feature card support
* Windows Sound System (WSS) support
* ESS ESFM native mode support
* Adlib Gold digital sound support (The TSR bundled with the original drivers isn't required; set the GOLD environment variable to the card's I/O port address)
* Fixed "Loading IWAD from sub-directory causes major errors" #279
* FDSetup has been upgraded to version 2.0 and can now be built using 100% open source tools. We have removed the dependencies on Laughing Dog for screen creation and Borland 4.x for compilation. Additionally, we developed PUPED, a new web based tool for easily creating PUP screens directly in your browser. Huge thanks to @jsmolina for porting the project to OpenWatcom, which helped us reduce the executable size by 20% in the process
* New FDSTART text mode launcher (based on the #78 idea, written in C): 
    * Pick any FastDoom executable or FDSETUP from an on-screen menu
    * Run benchmarks directly (FDBENCH is now gone)
    * Launch a single level (IWAD, PWADs, level and skill selection)
    * Built-in readme reader
    * Options menu with all the command line parameters available
* All projects (FASTDOOM, FDSETUP and FDSTART) can now be built using the build.sh script
* Added a 7th save slot
* Various fixes, optimizations and code cleanup
* Improved flat visplane colors by calculating a frequency histogram to find the most frequently used color. Also made flat and flatter visplane rendering a bit faster. Thanks @sandro-tishin! (#25 #282)
* Benchmark and frametime results are now saved in SYLK format instead of CSV (compatible with LibreOffice and Microsoft Excel, avoids CSV import problems)

## 1.3.0 (01 Jul 2026)
* Improved frame interpolation in uncapped framerate modes for smoother rendering #237 #251
* Fixed frame skipping in uncapped framerate modes when FPS drops below 35 #237 #251
* Added integer scaling support (up to 5×) for backbuffered VESA modes, enabling any VESA resolution in FastDoom

## 1.2.2 (20 May 2026)
* Fixed use-after-free bug in P_RunThinkers

## 1.2.1 (10 May 2026)
* Fixed automap level name not updating after completing a level (thanks y2k_se)
* Fixed game running too fast when starting new games or loading saves while running a benchmark (issue #267)

## 1.2.0 (19 Dec 2025)
* Fixed span rendering in backbuffered modes (potato detail and resolutions above 320×200)
* Minor optimizations to span rendering in backbuffered and VBE2 direct modes
* Added new span rendering routines for CPUs with slow SHLD performance (high detail)
* Added new column and span rendering routines optimized for CPUs that benefit from small code that fits in the L1 cache (high detail)
* Added new column and span rendering routines optimized for CPUs with MMX support (high detail)
* Optimized code paths for Intel Pentium, Pentium MMX, Pentium II, AMD K5, AMD K6, IDT WinChip, Rise MP6, Cyrix 6x86, and Cyrix 6x86MX CPUs in backbuffered and VBE2 direct modes
* Removed 386SX optimizations for backbuffered and VBE2 direct modes, as these modes are too demanding and the performance gains were negligible
* Minor BSP rendering optimizations, especially for AMD K6 and other CPUs that perform poorly with ROL/ROR instructions
* Switched NASM output from OBJ to COFF format, enabling larger ASM files to be built (some development builds failed at 1600×1200 resolutions)
* Optimized backbuffer-to-VRAM copy functions using MMX instructions
* Optimized backbuffer-to-VRAM copy functions using FPU instructions
* Added automatic CPU capability detection (FPU, MMX, etc.)

## 1.1.6 (27 Sep 2025)
* Fixed aspect ratio for VESA 1600x1200 mode (2D items)
* Fixed very dark colors on high gamma levels
* Optimized sprite clipping by Rav (vogons)
* Optimized mouse code
...
* Added IBM MDA Color support (works only on rev 0 cards)
